...
/*   Updated: 2024/02/20 14:48:51 by svogrig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pixel	projection(int x, int y, t_data data, t_transform *t)
{
	...
	pixel.x = round(t->scale * (t->a1 * x + t->a2 * y + t->a3 * z));
	pixel.y = round(t->scale * (t->b1 * x + t->b2 * y + t->b3 * z));
...
void	buffer_init(t_map *map, t_transform *transform)
{
	int	x;
